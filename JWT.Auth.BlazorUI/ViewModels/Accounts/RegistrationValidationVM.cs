    ...
    public class RegistrationValidationVM : AbstractValidator<RegistrationVM>
    {
        ...
        public RegistrationValidationVM(HttpClient httpClient)
        {
            ...
            RuleFor(x => x.Email)
                .NotEmpty()
                .EmailAddress()
                .MustAsync(async (value, CancellationToken) => await UniqueEmail(value)).WithMessage("Email is already exists.")
        ...
        private async Task<bool> UniqueEmail(string email)
        {
            try
            {
                string url = $"https://localhost:7057/api/User/unique-user-email?email={email}";
